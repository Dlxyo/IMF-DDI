...
def read_and_save_data_4entity(txt_file3, csv_file, output_file):
    ...
    Args:
        ...
        csv_file (str): Path to the CSV file with id, enzyme, target, gene, and disease columns.
...
def read_and_save_data(txt_file3, csv_file, output_file):
    ...
    Args:
        txt_file3 (str): Path to the text file containing part3 embeddings.
        csv_file (str): Path to the CSV file with id, enzyme, and target columns.
        output_file (str): Path for the output pickle file.
    '''
    data3 = read_float_array(txt_file3)
    df = pd.read_csv(csv_file)
...
def main():
    ...
    parser.add_argument('--four_entity', action='store_true',
                        help='Process id, enzyme, target, gene, and disease columns')
    parser.add_argument('--entity', action='store_true',
                        help='Process id, enzyme, and target columns only')
    args = parser.parse_args()

    if args.four_entity and args.entity:
        parser.error('Please specify only one of --four_entity or --entity')
    if args.four_entity:
        ...
        parser.error('Please specify --four_entity or --entity')
